...
class Shopware_Plugins_Frontend_PaymentSkrill_Bootstrap extends Shopware_Components_Plugin_Bootstrap
	...
	foreach ($elements as $element_id => $element_data)
	    {
            foreach (self::$paymentMethods as $pAbbrMethod => $pMethod)
                ...
                $pMethodElement = new Shopware_Components_PaymentSkrill_Checkbox('skrill_' .
                                                                                strtolower($pAbbrMethod),
										$pluginId);
    ...
    protected function createPayments()
        ...
        if (!$payment)
            {
            Shopware()->Payments()->createRow(array(
                                    'name' => 'skrill',
                                    'description' => 'Skrill',
    ...
    public function install ()
        {
        Shopware()->Loader()->registerNamespace('Shopware_Components_PaymentSkrill',
    						dirname(__FILE__) . '/Components/Skrill/');
	...
	$event = $this->createEvent('Enlight_Controller_Dispatcher_ControllerPath_Frontend_PaymentSkrill',
				    'onGetControllerPath');
	$this->subscribeEvent($event);
	...
	foreach (self::$paymentMethods as $pAbbrMethod => $pMethod)
	    ...
	    $pMethodNew = new Shopware_Components_PaymentSkrill_Checkbox(
						'skrill_' . strtolower($pAbbrMethod));
    ...
    public function Payment()
        {
	return Shopware()->Payments()->fetchRow(
		array('name=?' => 'skrill'));
        }
	...
	foreach (self::$paymentMethods as $pAbbrMethod => $pMethod)
	    {
	    $pMethodElement = new Shopware_Components_PaymentSkrill_Checkbox(
								    'skrill_' . $pAbbrMethod,
								    $this->getId());
	    $pMethodElement->setLabel($pMethod);
            ...
            $form->setElement('checkbox', $pMethodElement->name, array(
                            'label' => $pMethodElement->description,
                            'value' => false
                            ));
	    }
